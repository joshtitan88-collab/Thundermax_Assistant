...
function renderBike(card, p) {
  ...
  if (prof.injectors) {
    const inj = typeof prof.injectors === 'string' ? prof.injectors : `${prof.injectors.flow_g_s || ''} g/s`;
    rows.append(el('div', { class: 'row' }, el('span', {}, 'Injectors'), el('span', { class: 'lim' }, inj)));
  }
  if (prof.current_flash_file) row('Flash-first', prof.current_flash_file);
  card.append(rows);
}
...
function renderSafety(card, g) {
  ...
  rows.append(el('div', { class: 'row' }, el('span', {}, '.tbw files'), el('span', { class: 'lim' }, 'NEVER written by this app')));
  card.append(rows);
}
